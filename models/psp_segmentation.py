...
class PSP_segmentation(nn.Module):
    ...
    def __init__(self, cfg, cfg_model, writer, logger):
        ...
        """
        super(PSP_segmentation,self).__init__()  # call the initialization method of BaseModel
        ...
        self.ouput_size = ((cfg['data']['target']['img_rows'],cfg['data']['target']['img_cols']) if cfg_model.get('type') == 'cityscapes'
            else (cfg['data']['source']['img_rows'],cfg['data']['source']['img_cols']))
        # self.ouput_size = (cfg['data']['target']['img_rows'],cfg['data']['target']['img_cols'])
        ...
        self.dropout_1 = nn.Dropout2d(p=0.15, inplace=False)
        self.dropout_2 = nn.Dropout2d(p=0.1, inplace=False)
        self.classification_1 = nn.Conv2d(512, 128, 1, 1, 0)
        self.classification_2 = nn.Conv2d(128, self.n_classes, 1, 1, 0)
        self.conv = nn.Conv2d(self.n_classes, self.n_classes, 1,)
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, size=(int(self.ouput_size[0]/4), int(self.ouput_size[0]/4)), mode='bilinear', align_corners=True)
        x = self.dropout_2(x)
        x = self.classification_2(x)
        x = F.interpolate(x, size=self.ouput_size, mode='bilinear', align_corners=True)
        x = self.conv(x)
        return x
